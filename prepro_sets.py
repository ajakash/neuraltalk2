...
example element in this list would look like (NO!)
...
/images is (K,3,256,256) uint8 array of raw image data in RGB format
/set_start_ix and /set_end_ix are (N,) uint32 arrays of pointers to the
    first and last indices (in the range 1..K) of images per each of N sets
/labels is (M,max_length) uint32 array of encoded labels, zero padded
/label_start_ix and /label_end_ix are (N,) uint32 arrays of pointers to the 
  first and last indices (in range 1..M) of labels (which together forms the
  full description) for each image
